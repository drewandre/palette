...
class Slider extends Component {
	...
	handleMouseDown(e) {
		const { top, height } = this.potar.getBoundingClientRect();
	...
	render() {
		...
		const strokeDashoffset =
			value < start ? length * (percent + origin) : length * origin;
      ...
      A ${radius} ${radius} 0 ${angle < 0 ? 0 : 1} 1 ${diameter - x} ${y}`;
		...
		return (
			<svg
				...
				viewBox={`${-halfBorder} ${-halfBorder} ${diameter +
					border} ${diameter + border}`}
			...
			>
				...
				>
					{this.props.fixedSliderValue
						? this.props.fixedSliderValue.toFixed(0)
						: null}
				</text>
				<path className={styles.sliderBar} style={{ strokeWidth }} d={d} />
				<path
