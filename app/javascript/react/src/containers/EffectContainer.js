...
class EffectContainer extends Component {
		...
		fetch(
			...
			.then(body => {
				...
				return fetch(
					`/api/v1/users/${nextUser.handle}/products/${
						nextUser.current_product_name
					}/effect_settings/${body.effect_name}`
				);
			})
			.then(response => response.json())
			.then(body => {
	...
	postSliderValuesToFetch() {
		if (Date.now() - this.state.lastKeyPressedTime > 500) {
			...
			fetch(
				...
				}/effect_settings/${this.state.active_effect}`,
				{
	...
	handleSlider(sliderValue) {
		...
		var parameterValueIndex = this.state.effectParameterNames.indexOf(
			parameterName
		);
		let effectParameterValues = this.state.effectParameterValues;
		if (~parameterValueIndex) {
			effectParameterValues[parameterValueIndex] = sliderValue;
		}
		this.setState({
